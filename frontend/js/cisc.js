let ciscInicializado = false;
let ciscEjecutando = false;
let ciscTimers = [];
...
function inicializarCisc() {
  if (ciscInicializado) return;
  ciscInicializado = true;

  cargarModuloCisc();

  document.getElementById("btnEjecutarCisc")
    ?.addEventListener("click", ejecutarCisc);

  document.getElementById("btnReiniciarCisc")
...
function obtenerDatosEntradaCisc() {
  return {
    A: Number(document.getElementById("inputA_Cisc")?.value || 0),
    B: Number(document.getElementById("inputB_Cisc")?.value || 0),
    C: Number(document.getElementById("inputC_Cisc")?.value || 0),
    D: Number(document.getElementById("inputD_Cisc")?.value || 0)
  };
}

async function ejecutarCisc(event) {
  event?.preventDefault();

  if (ciscEjecutando) return;

  const datos = obtenerDatosEntradaCisc();

  detenerAnimacionCisc();
  limpiarCisc(false);

  ciscEjecutando = true;
  cambiarBotonCisc(true);
  ...
  animarCisc(datos, respuesta);
}

function animarCisc(datos, respuesta) {
  let paso = 0;

  const pasos = [
    () => {
      registrosCisc.AX = datos.A;
    ...
    () => {
      ...
      registrosCisc.AX = respuesta.suma ?? (datos.A + datos.B);
      actualizarUnidadCisc("ADD", antes, datos.B, registrosCisc.AX);
      agregarHistorialCisc(++paso, instruccionesCisc[1], "Suma directa con memoria", `AX = ${registrosCisc.AX}`);
    },
    () => {
      registrosCisc.BX = datos.C;
    ...
    () => {
      ...
      registrosCisc.BX = respuesta.resta ?? (datos.C - datos.D);
      actualizarUnidadCisc("SUB", antes, datos.D, registrosCisc.BX);
      agregarHistorialCisc(++paso, instruccionesCisc[3], "Resta directa con memoria", `BX = ${registrosCisc.BX}`);
    },
    () => {
      const antes = registrosCisc.AX;
      registrosCisc.AX = respuesta.resultado ?? (registrosCisc.AX * registrosCisc.BX);
    ...
    () => {
      memoriaCisc["0x110"].valor = respuesta.resultado ?? registrosCisc.AX;
      document.getElementById("resultadoCisc").textContent = memoriaCisc["0x110"].valor;

      localStorage.setItem("resultadoCisc", JSON.stringify({
        arquitectura: "CISC",
        instrucciones: respuesta.instrucciones ?? 6,
        memoria: respuesta.memoria ?? 4,
        alu: respuesta.alu ?? 3,
        registros: 2,
        complejidad: 90,
        pipeline: 65,
        resultado: memoriaCisc["0x110"].valor
      }));

      agregarHistorialCisc(++paso, instruccionesCisc[5], "Store directo a memoria", `Memoria[0x110] = ${memoriaCisc["0x110"].valor}`);
      finalizarCisc();
    }
  ];

  pasos.forEach((accion, index) => {
    ...
    ciscTimers.push(timer);
  });
}

function finalizarCisc() {
  ciscEjecutando = false;
  cambiarBotonCisc(false);
}

function cambiarBotonCisc(ejecutando) {
  const btn = document.getElementById("btnEjecutarCisc");
  if (!btn) return;

  btn.disabled = ejecutando;
  btn.textContent = ejecutando ? "Ejecutando..." : "Ejecutar";
}

function detenerAnimacionCisc() {
  ciscTimers.forEach(timer => clearTimeout(timer));
  ciscTimers = [];
}

function limpiarCisc(mantenerEntradas = true) {
  const datos = obtenerDatosEntradaCisc();
...
function reiniciarCisc(event) {
  event?.preventDefault();

  detenerAnimacionCisc();
  finalizarCisc();

  document.getElementById("inputA_Cisc").value = 12;
  document.getElementById("inputB_Cisc").value = 8;
  document.getElementById("inputC_Cisc").value = 15;
  document.getElementById("inputD_Cisc").value = 10;
